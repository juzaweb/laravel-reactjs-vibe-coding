...
export const Checkbox = forwardRef<HTMLInputElement, CheckboxProps>(
  ({ label, description, error, className = '', wrapperClassName = '', id, ...props }, ref) => {
    ...
    const checkboxId = id || props.name || uniqueId;
      ...
      <div className={`flex items-start mb-4 ${wrapperClassName}`}>
        <div className="flex items-center h-5">
          <input
            ...
            ref={ref}
            className={`
              w-4 h-4 border rounded bg-transparent focus:ring-2 focus:ring-offset-0 transition-colors
        ...
        {(label || description) && (
          <div className="ml-3 text-sm">
            {label && (
              <label htmlFor={checkboxId} className="font-medium text-[var(--text-main)]">
                {label}
              </label>
            )}
            {description && !error && (
