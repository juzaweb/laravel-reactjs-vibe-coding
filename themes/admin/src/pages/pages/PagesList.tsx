...
import { usePages, useDeletePage, useBulkPages } from './hooks';
...
export const PagesList: React.FC = () => {
  ...
  const bulkPagesMutation = useBulkPages();
  ...
  const handleBulkAction = async () => {
    ...
    if (bulkAction === 'delete' && !window.confirm(t('are_you_sure_delete_selected_pages', 'Are you sure you want to delete selected pages?'))) {
      return;
    }

    try {
      await bulkPagesMutation.mutateAsync({ ids: selectedIds, action: bulkAction });
      setSelectedIds([]);
      setBulkAction('');
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm(t('are_you_sure_delete_page', 'Are you sure you want to delete this page?'))) {
      try {
        ...
        toast.success(t('success_deleting_page', 'Page deleted successfully'));
      } catch (err) {
        console.error('Failed to delete page:', err);
        toast.error(t('error_deleting_page', 'Error deleting page'));
      }
    }
  };
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6">
      <PageHeader
        title={t('pages', 'Pages')}
        breadcrumbs={[{ label: t('pages', 'Pages') }]}
        actions={
      ...
      <div className="bg-[var(--bg-card)] rounded-xl shadow-sm border border-[var(--border-color)] overflow-hidden">
        ...
        <div className="p-4 border-b border-[var(--border-color)] flex items-center gap-2">
          ...
          >
            ...
            {hasPermission('pages.delete') && <option value="delete">{t('delete', 'Delete')}</option>}
          </select>
          <Button
            variant="outline"
            size="sm"
            onClick={handleBulkAction}
            disabled={!bulkAction || selectedIds.length === 0 || bulkPagesMutation.isPending}
          >
            {t('apply', 'Apply')}
          </Button>
                ...
                <tr key={page.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                      ...
                      {page.status ? t(`status_${page.status}`, page.status) : t('status_unknown', 'Unknown')}
                    </span>
                  </td>
