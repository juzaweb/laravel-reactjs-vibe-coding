...
export const PostsList: React.FC = () => {
  ...
  const bulkPostsMutation = useBulkPosts();
  ...
  const handleBulkAction = async () => {
    ...
    if (bulkAction === 'delete' && !window.confirm(t('are_you_sure_delete_selected_posts', 'Are you sure you want to delete selected posts?'))) {
      return;
    }

    try {
      await bulkPostsMutation.mutateAsync({ ids: selectedIds, action: bulkAction });
      setSelectedIds([]);
      setBulkAction('');
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm(t('are_you_sure_delete_post', 'Are you sure you want to delete this post?'))) {
      try {
        await deletePostMutation.mutateAsync(id);
        toast.success(t('success_deleting_post', 'Post deleted successfully'));
      } catch (err) {
        console.error('Failed to delete post:', err);
        toast.error(t('error_deleting_post', 'Error deleting post'));
      }
    }
  };
  ...
  if (error) {
    return <div className="p-6 text-center text-red-500">{t('error_loading_posts', 'Error loading posts')}</div>;
  }
      ...
      <div className="bg-[var(--bg-card)] rounded-xl shadow-sm border border-[var(--border-color)] overflow-hidden">
        ...
        <div className="p-4 border-b border-[var(--border-color)] flex items-center gap-2">
          ...
          >
            ...
            {hasPermission('posts.delete') && <option value="delete">{t('delete', 'Delete')}</option>}
          </select>
          <Button
            variant="outline"
            size="sm"
            onClick={handleBulkAction}
            disabled={!bulkAction || selectedIds.length === 0 || bulkPostsMutation.isPending}
          >
            {t('apply', 'Apply')}
          </Button>
              ...
              {data?.data?.map((post) => (
                <tr key={post.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-[var(--text-main)]">{post.title}</div>
                    <div className="text-sm text-[var(--text-muted)]">{post.slug}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                      ...
                      {post.status ? t(`status_${post.status}`, post.status) : t('status_unknown', 'Unknown')}
                    </span>
                  </td>
              ...
              {(!data?.data || data.data.length === 0) && (
                <tr>
                  <td colSpan={4} className="px-6 py-8 text-center text-[var(--text-muted)]">
                    {t('no_posts_found', 'No posts found')}
                  </td>
                </tr>
              )}
            </tbody>
